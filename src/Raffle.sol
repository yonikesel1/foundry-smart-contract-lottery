...
contract Raffle is VRFConsumerBaseV2 {
    error Raffle__NotEnoughETHForFee();
    error Raffle__RaffleNotOpen();
    error Raffle__TransferFailed();
    error Raffle__UpkeepNotNeeded(uint256 currentContractBalance, uint256 numPlayers, RaffleState raffleState);
    ...
    event RequesttedRaffleWinner(uint256 requestId);
    ...
    function enterRaffle() external payable {
        if (msg.value < i_entranceFee) {
            revert Raffle__NotEnoughETHForFee();
        }
    ...
    function performUpkeep(bytes memory /* performData */ ) external {
        ...
        emit RequesttedRaffleWinner(requestId);
        console.log("RequesttedRaffleWinner requestId: ", requestId);
    }

    function fulfillRandomWords(uint256, /*_requestId*/ uint256[] memory _randomWords) internal override {
        // Checks (if -> error)
        // Effects (with our contract)
    ...
    function getPlayersAmount() external view returns (uint256) {
